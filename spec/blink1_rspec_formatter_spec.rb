...
describe Blink1RspecFormatter do
  ...
  before do
    expect_any_instance_of(Kernel).to receive(:sleep).with(Blink1RspecFormatter::BLINK_TIMES)
  ...
  it 'execute success spec is blink SUCCESS.' do
    expect(blink1_mock).to receive(:set_rgb).with(
                               Blink1RspecFormatter::COLOR_SUCCESS[0],
                               Blink1RspecFormatter::COLOR_SUCCESS[1],
                               Blink1RspecFormatter::COLOR_SUCCESS[2]
                           )

    output = 'dummy'
    formatter = Blink1RspecFormatter.new(output)
  ...
  it 'execute error spec is blink ERROR.' do
    ...
    expect(blink1_mock).to receive(:blink).with(
                               Blink1RspecFormatter::COLOR_FAIL[0],
                               Blink1RspecFormatter::COLOR_FAIL[1],
                               Blink1RspecFormatter::COLOR_FAIL[2],
                               3
                           )
  ...
  it 'execute pending spec is blink PENDING.' do
    ...
    expect(blink1_mock).to receive(:blink).with(
                               Blink1RspecFormatter::COLOR_PENDING[0],
                               Blink1RspecFormatter::COLOR_PENDING[1],
                               Blink1RspecFormatter::COLOR_PENDING[2],
                               3
                           )
